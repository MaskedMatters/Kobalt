  ...
  {:else}
    <div class="projects-grid">
      {#each $projects as project}
        <md-outlined-card style="border-radius: var(--md-sys-shape-corner-large); box-shadow: 0 2px 8px rgba(103,80,164,0.08);">
          <div style="padding: 1.5rem;">
            <h2 style="font-size: 1.5rem; font-weight: 600; margin-bottom: 0.5rem; color: var(--md-sys-color-on-surface);">{project.title}</h2>
            <p style="color: var(--md-sys-color-on-surface-variant); margin-bottom: 0.5rem;">{project.description}</p>
            <span style="display: inline-block; padding: 0.25rem 0.75rem; border-radius: 1rem; background: var(--md-sys-color-secondary-container); color: var(--md-sys-color-on-secondary-container); font-size: 0.95rem; font-weight: 500;">{project.status}</span>
          </div>
        </md-outlined-card>
      {/each}
    </div>
...
<md-filled-button class="m3-fab" aria-label="Create Project" on:click={openCreateProject} style="border-radius: 50%; width: 56px; height: 56px; font-size: 2rem; min-width: 56px; min-height: 56px; padding: 0; display: flex; align-items: center; justify-content: center;">
  +
</md-filled-button>
...
{#if $showCreateModal}
  <div class="m3-modal-backdrop" on:click|self={closeCreateProject}>
    <form class="m3-modal" on:submit|preventDefault={createProject}>
      ...
      {/if}
      <div class="actions">
        <md-filled-button type="button" on:click={closeCreateProject} disabled={creating}>Cancel</md-filled-button>
        <md-filled-button type="submit" disabled={creating}>{creating ? 'Creating...' : 'Create'}</md-filled-button>
      </div>
    </form>
  </div>
{/if} 